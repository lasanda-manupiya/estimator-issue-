                    ...
                    <div class="pull-rigth">
                        @if(@$project)
                            ...
                            @if (auth()->user()->can('access', 'carbontoolkit add'))
                              ...
                              <a href="{{ route('carbon.save.project', $project->id) }}" class="btn btn-secondary btn-sm">Save Project</a>&nbsp;
                            @endif
                            <a href="{{ route('carbon.excel.expanded', $project->id) }}" class="btn btn-warning btn-sm">Excel Expanded</a>&nbsp;
                            <a href="{{ route('carbon.excel.collapsed', $project->id) }}" class="btn btn-danger btn-sm">Excel Collapsed</a>&nbsp;
                            <a href="javascript:;" data-project-id="{{ $project->id }}" data-route="{{ route('carbon.ajax.project.estimate.detail', $project->id) }}" class="btn btn-success btn-sm show-project-info">Show Detail</a>&nbsp;
                            <a href="javascript:;" data-project-id="{{ $project->id }}" data-route="{{ route('carbon.ajax.project.formula.detail', $project->id) }}" class="btn btn-dark btn-sm show-project-formula">Formula</a>&nbsp;
                        @else
            ...
            <div class="card project-estimate-detail" style="display:none">
                {{ Form::open(['route' => 'estimates.projects.update.detail']) }}
                    ...
                    <div class="card-header">
                        <h3 class="card-title">Project Carbon Toolkit Detail</h3><hr>
        ...
        {{ Form::select('project_ids', $allProjects,$user->default_project, [
            ...
            'data-live-search'=>'true', 
                ...
                <hr><br><div class="col-md-12">
                         ...
                         @if(auth()->user()->can('access', 'carbontoolkit add'))
                            <a href="{{ route('carbon.projects.import.view') }}" class="mb-2 mr-2 btn btn-info btn-sm">
                                Import Carbon Toolkit
                            </a>
                        @endif
                ...
                <div class="card-header">
                    <h3 class="card-title">Project Carbon Toolkit</h3><hr>
                         </div> 
                ...
                <div class="card-body table-responsive">
                    @include('layouts.flash.alert')
                     <div class="col-md-6">
						   ...
						   <input id="myInput" type="text" onKeyUp="myFunction()">&nbsp&nbsp&nbsp<a href="{{ route('carbon.projects') }}" class="mb-2 mr-2 btn btn-info btn-sm">
                                Back&nbsp
                            </a>
                    ...
                    <table class=" table table-bordered main-activity-table" id="main_activity_table" style="width:110%">
                        <thead class="table-success" style="background-color: #84c1ff;" >
                            <tr>
                                ...
                                <th width="4%">Unit</th>
                                <th width="4%">Total Co<sub>2</sub></th>
                                <th width="1%">
                                    @if (auth()->user()->can('access', 'carbontoolkit add'))
                                        <a title="Add main activity" href="javascript:;" class="btn btn-sm btn-success add-main-activity-row" data-project-id="{{ $project->id }}" data-route="{{ route('carbon.ajax.add.main.activity.row', $project->id) }}">
                                            <i class="fas fa-plus"></i>
                                        </a>
                                    @endif
                                </th>
                            ...
                            @if($mainActivities->isNotEmpty())
                                @foreach($mainActivities as $mainActivity)
                                    <tr class="main-activity-row-{{ $mainActivity->id }} expandable" data-row-id="{{ $mainActivity->id }}">
                                        ...
                                        <td>
                                            <input title="Unit Rate- {{ round($mainActivity->unit_rate,2) }}"  type="text" name="main_activity[{{ $mainActivity->id }}][unit_rate]" value="{{ round($mainActivity->unit_rate,2) }}" id="unit_rate_main{{ $mainActivity->id }}" class="form-control unit_rate_main" onkeypress="javascript:return isNumber(event)" disabled="disabled" onchange="update_main_activity_row('{{ route('carbon.ajax.update.project.main.activity.row', $mainActivity->id) }}', 'unit_rate', this.value, '{{ $mainActivity->id }}');">
                                        </td>
                                        <td>
                                            <input title="Unit- {{ $mainActivity->unit }}" type="text" name="main_activity[{{ $mainActivity->id }}][unit]" value="{{ $mainActivity->unit }}" id="unit_main{{ $mainActivity->id }}" class="form-control unit_main" onchange="update_main_activity_row('{{ route('carbon.ajax.update.project.main.activity.row', $mainActivity->id) }}', 'unit', this.value, '{{ $mainActivity->id }}');">
										...
										<td>
                                            <input title="Total Co2- {{ round($mainActivity->totalco,2) }}"  type="text" name="main_activity[{{ $mainActivity->id }}][totalco]" value="{{ round($mainActivity->totalco,2) }}" id="totalco_main{{ $mainActivity->id }}" class="form-control totalco" onkeypress="javascript:return isNumber(event)" disabled="disabled" onchange="update_main_activity_row('{{ route('carbon.ajax.update.project.main.activity.row', $mainActivity->id) }}', 'totalco', this.value, '{{ $mainActivity->id }}');">
                                        </td>
                                        <td>
                                            @if(auth()->user()->can('access', 'carbontoolkit add'))
                                                    ...
                                                    @if($mainActivity->subActivities->isNotEmpty())
                                                        @foreach($mainActivity->subActivities as $subActivity)
                                                            <tr class="sub-activity-row-{{ $subActivity->id }} expandable" data-row-id="{{ $subActivity->id }}">
                                                                ...
                                                                <td><input type="button" class="btn btn-primary btn-sm expandable-input" value="+"></td>
                                                                <td>
                                                                <input title="Sub Code- {{ $subActivity->sub_code }}" type="text" name="sub_activity[{{ $subActivity->id }}][sub_code]" value="{{ $subActivity->sub_code }}" id="sub_code{{ $subActivity->id }}" class="form-control sub_code" disabled="disabled" onchange="update_sub_activity_row('{{ route('carbon.ajax.update.project.sub.activity.row', $subActivity->id) }}', 'sub_code', this.value, '{{ $subActivity->id }}', '{{ $subActivity->library_main_activity_id }}');">
                                                                </td>
                                                                
                                                                <td>
                                                                <input title="Activity- {{ $subActivity->activity }}" type="text" name="sub_activity[{{ $subActivity->id }}][activity]" value="{{ $subActivity->activity }}" id="activity_sub{{ $subActivity->id }}" class="form-control activity_sub" onchange="update_sub_activity_row('{{ route('carbon.ajax.update.project.sub.activity.row', $subActivity->id) }}', 'activity', this.value, '{{ $subActivity->id }}', '{{ $subActivity->library_main_activity_id }}');">
                                                                </td>
                                                                <td>
                                                                 <input title="Quantity- {{ $subActivity->quantity }}"  type="text" name="sub_activity[{{ $subActivity->id }}][quantity]" value="{{ $subActivity->quantity }}" id="quantity_sub{{ $subActivity->id }}" class="form-control quantity_sub" onkeypress="javascript:return isNumber(event)" onchange="update_sub_activity_row('{{ route('carbon.ajax.update.project.sub.activity.row', $subActivity->id) }}', 'quantity', this.value, '{{ $subActivity->id }}', '{{ $subActivity->library_main_activity_id }}');">
                                                                </td>
                                                                <td>
																<input title="Carbon- {{ $subActivity->foot_print_value }}" type="text" name="sub_activity[{{ $subActivity->id }}][foot_print_value]" value="{{ $subActivity->foot_print_value }}" id="foot_print_value_sub{{ $subActivity->id }}" class="form-control foot_print_value_sub" onchange="update_sub_activity_row('{{ route('carbon.ajax.update.project.sub.activity.row', $subActivity->id) }}', 'foot_print_value', this.value, '{{ $subActivity->id }}', '{{ $subActivity->library_main_activity_id }}');">
                                                                </td>
                                                                <td>
                                                                <input title="Rate- {{ round($subActivity->rate,2) }}" type="text" name="sub_activity[{{ $subActivity->id }}][rate]" value="{{ round($subActivity->rate,2) }}" id="rate_sub{{ $subActivity->id }}" class="form-control rate_sub" onkeypress="javascript:return isNumber(event)" onchange="update_sub_activity_row('{{ route('carbon.ajax.update.project.sub.activity.row', $subActivity->id) }}', 'rate', this.value, '{{ $subActivity->id }}', '{{ $subActivity->library_main_activity_id }}');">
                                                                </td>
                                                                <td>
                                                                <input title="Total- {{ round($subActivity->total,2) }}"  type="text" name="sub_activity[{{ $subActivity->id }}][total]" value="{{ round($subActivity->total,2) }}" id="total_sub{{ $subActivity->id }}" class="form-control total_sub" onkeypress="javascript:return isNumber(event)" disabled="disabled" onchange="update_sub_activity_row('{{ route('carbon.ajax.update.project.sub.activity.row', $subActivity->id) }}', 'total', this.value, '{{ $subActivity->id }}', '{{ $subActivity->library_main_activity_id }}');">
                                                                </td>
                                                                <td>
                                                                <input title="Unit- {{ $subActivity->unit }}" type="text" name="sub_activity[{{ $subActivity->id }}][unit]" value="{{ $subActivity->unit }}" id="unit_sub{{ $subActivity->id }}" class="form-control unit_sub" onchange="update_sub_activity_row('{{ route('carbon.ajax.update.project.sub.activity.row', $subActivity->id) }}', 'unit', this.value, '{{ $subActivity->id }}', '{{ $subActivity->library_main_activity_id }}');">
                                                                
                                                                <input type="hidden" name="sub_activity[{{ $subActivity->id }}][id]" value="{{ $subActivity->id }}">
                                                                <input type="hidden" name="sub_activity[{{ $subActivity->id }}][hr]" value="{{ $subActivity->hr }}" id="hr_sub{{ $subActivity->id }}" class="form-control hr_sub" onchange="update_sub_activity_row('{{ route('carbon.ajax.update.project.sub.activity.row', $subActivity->id) }}', 'hr', this.value, '{{ $subActivity->id }}', '{{ $subActivity->library_main_activity_id }}');">
                                                                <input type="hidden" name="sub_activity[{{ $subActivity->id }}][mhr]" value="{{ $subActivity->mhr }}" id="mhr_sub{{ $subActivity->id }}" class="form-control mhr_sub" onchange="update_sub_activity_row('{{ route('carbon.ajax.update.project.sub.activity.row', $subActivity->id) }}', 'mhr', this.value, '{{ $subActivity->id }}', '{{ $subActivity->library_main_activity_id }}');">
                                                                <input type="hidden" name="sub_activity[{{ $subActivity->id }}][total_hr]" value="{{ $subActivity->total_hr }}" id="total_hr_sub{{ $subActivity->id }}" class="form-control total_hr_sub" onchange="update_sub_activity_row('{{ route('carbon.ajax.update.project.sub.activity.row', $subActivity->id) }}', 'total_hr', this.value, '{{ $subActivity->id }}', '{{ $subActivity->library_main_activity_id }}');">
                                                                <input type="hidden" name="sub_activity[{{ $subActivity->id }}][total_mhr]" value="{{ $subActivity->total_mhr }}" id="total_mhr_sub{{ $subActivity->id }}" class="form-control total_mhr_sub" onchange="update_sub_activity_row('{{ route('carbon.ajax.update.project.sub.activity.row', $subActivity->id) }}', 'total_mhr', this.value, '{{ $subActivity->id }}', '{{ $subActivity->library_main_activity_id }}');">
                                                                </td>
                                                                <td>
																<input title="Total Co2- {{ round($subActivity->totalco,2) }}" type="text" name="sub_activity[{{ $subActivity->id }}][totalco]" value="{{ round($subActivity->totalco,2) }}" id="totalco_sub{{ $subActivity->id }}" class="form-control totalco_sub" onkeypress="javascript:return isNumber(event)" onchange="update_sub_activity_row('{{ route('carbon.ajax.update.project.sub.activity.row', $subActivity->id) }}', 'totalco', this.value, '{{ $subActivity->id }}', '{{ $subActivity->library_main_activity_id }}');">
                                                                </td>
                                                                <td>
                                                                @if(auth()->user()->can('access', 'carbontoolkit add'))
                                                                <a title="Delete sub activity" href="javascript:;" class="btn btn-sm btn-danger remove-sub-activity-row" data-sub-activity-id="{{ $subActivity->id }}" data-route="{{ route('carbon.ajax.delete.sub.activity.row', $subActivity->id) }}">
                                                                <i class="fas fa-trash"></i>
                                                                </a>
                                                                @endif
                                                                </td>
                                                                ...
                                                                <td colspan="8" class="expandable">
                                                                    <table class="table table-bordered activity-table" id="activity-table{{ $subActivity->id }}">
                                                                        <thead class="table-warning" style="background-color: #285481; color: white;">
                                                                            ...
                                                                            <th>Activity</th>
                                                                            <th width="7%">Unit</th>
                                                                            <th width="7%">Co<sub>2</sub>&nbsp Unit</th>
                                                                            <th width="7%">Quantity</th>
                                                                            <th>Cost Rate</th>
                                                                            <th>Selling Rate</th>
                                                                            <th>Profit</th>
                                                                            ...
                                                                            <th>
                                                                                @if(auth()->user()->can('access', 'carbontoolkit add'))
                                                                                    <a href="javascript:;" class="btn btn-sm btn-success add-activity-row" data-sub-activity-id="{{ $subActivity->id }}" data-route="{{ route('carbon.ajax.add.activity.row', $subActivity->id) }}">
                                                                                        <i class="fas fa-plus"></i>
                                                                                    </a>
                                                                                @endif
                                                                            </th>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(document).on('keyup', '.activity-rate, .activity-quantity, .activity-total,.activity-foot_print_value,.activity-totalco', function()  {  
            //alert("712");  
			var tr = $(this).parent().parent();  
            var index = tr.attr('data-row-id');
			...
			//alert(quantity);
            var foot_print_value = $('#foot_print_value'+index).val();
            ...
            foot_print_value = isNaN(parseFloat(foot_print_value)) ? 0 : parseFloat(foot_print_value);
            ...
            foot_print_value = parseFloat(foot_print_value).toFixed(2);
            ...
            var totalco=foot_print_value*quantity;
                ...
                if (t.tagName && t.tagName.toLowerCase() == tagName) {
                    ...
                    var tables = document.getElementById(t.id);
                        ...
                        for(i = 1; i < rows.length; i++)
                        {
                            ...
                            var foot_print_value = $('#foot_print_value'+indexs).val();
                            ...
                            foot_print_value = parseFloat(foot_print_value);
                            totalco = parseFloat(totalco);
                            trate = trate+rate;
                            tquantity = tquantity+quantity;
                            ttotal = ttotal+total;
                            tfoot_print_value = tfoot_print_value+foot_print_value;
                            ...
                            if(l == c)
                            {
                                ...
                                $('#foot_print_value_sub'+totalcos).val(ttotalco);
								//alert(ttotalco);
                                var qso = $('#quantity_sub'+totalcos).val();
                                qso=parseFloat(qso);
                                var tttotalco=qso*ttotalco;
                                tttotalco = parseFloat(tttotalco).toFixed(2);		
                                $('#totalco_sub'+totalcos).val(tttotalco);
                        ...
                        while (tdd) {
                            ...
                            { 
                                ...
                                if(indexs1 === undefined){
                                    ...
                                    var foot_print_value1 = $('#foot_print_value_sub'+indexs1).val();
                                    ...
                                    if(rate1 === undefined && quantity1 === undefined && total1 === undefined && foot_print_value1 === undefined && totalco1 === undefined){
                                 
                                    }else{
                                        rate1 = parseFloat(rate1);
                                        quantity1 = parseFloat(quantity1);
                                        total1 = parseFloat(total1);
                                        foot_print_value1 = parseFloat(foot_print_value1);
                                        ...
                                        tfoot_print_value1 = tfoot_print_value1+foot_print_value1;
                                        thr1 = thr1+total_hr1;
                                ...
                                if(l1 == c1){
                                    ...
                                    $('#foot_print_value_main'+totals1).val(ttotalco1);
                                    var qm =$('#quantity_main'+totals1).val();
                                    qm=parseFloat(qm);
        ...
        $(document).on('keyup','.rate_sub, .quantity_sub, .total_sub,.foot_print_value_sub,.totalco_sub', function(){  
            ...
            var foot_print_value_sub = $('#foot_print_value_sub'+index).val();
            ...
            foot_print_value_sub = isNaN(parseFloat(foot_print_value_sub)) ? 0 : parseFloat(foot_print_value_sub);
            ...
            foot_print_value_sub = parseFloat(foot_print_value_sub).toFixed(2);
            ...
            var totalco_sub=foot_print_value_sub*quantity_sub;
                ...
                if (t.tagName && t.tagName.toLowerCase() == tagName) {
                    ...
                    for(i = 1; i < rows.length; i++){
                        ...
                        if(indexs === undefined){
                            ...
                            var foot_print_value=$('#foot_print_value_sub'+indexs).val();
                            ...
                            if(rate === undefined && quantity === undefined && total === undefined && foot_print_value === undefined && totalco === undefined){ 
                        ...
                        if(l == c){
							...
							ttotalco = parseFloat(ttotalco).toFixed(2);		
                            $('#foot_print_value_main'+totals).val(ttotalco);
                            var qm = $('#quantity_main'+totals).val();
                            qm=parseFloat(qm);
                            var ttotals = qm*ttotalco;
                            ttotals = parseFloat(ttotals).toFixed(2);		
                            $('#totalco_main'+totals).val(ttotals);
        ...
        $(document).on('keyup', '.rate_main, .quantity_main, .total_main, .unit_qty_main,.foot_print_value_main,.totalco_main', function(){  
            ...
            var foot_print_value_main=$('#foot_print_value_main'+index).val();
            ...
            foot_print_value_main = isNaN(parseFloat(foot_print_value_main)) ? 0 : parseFloat(foot_print_value_main);
            ...
            foot_print_value_main = parseFloat(foot_print_value_main).toFixed(2);
            ...
            var totalco_main=foot_print_value_main*quantity_main;
    ...
    function update_activity_row(route, colomn_name, colomn_value, row_id, sub_row_id, main_row_id){
        ...
        if(colomn_name=="quantity" || colomn_name=="rate" || colomn_name=="total" || colomn_name=="unit" || colomn_name=="unit_qty" || colomn_name=="totalco"){
	...
	if(employee.length <=0) {
		...
		var table = "library_main_activities";
			...
			$.ajax({
				...
				url:"{{ route('carbon.ajax.delete.main.activity') }}",
				method:"POST",  
        	...
        	if(proj =="") {
        		...
        		var table = "library_main_activities";
        			...
        			$.ajax({
        				...
        				url:"{{ route('carbon.copy.lib_estimate') }}",
        				method:"POST",  
